...
try {
    ...
    // Validate cơ bản
    if(empty($title)) throw new Exception("Tên sách không được để trống.");
    if($category_id <= 0 && empty($new_category)) throw new Exception("Chọn thể loại hoặc thêm mới");
    if($author_id <= 0 && empty($new_author_name)) throw new Exception("Chọn tác giả hoặc thêm mới");
